...
- **API Server Routes:**
    ...
    - `GET /api/sports/draft/:league?year=YYYY`: Multi-sport draft center (NFL/NBA/NHL/MLB). Returns picks (with team/athlete/grade), teams (with positional needs and next pick), top prospects (with grades/rankings/headshots), positions, and draft status (pre/in/post). Caches 30s during live draft, 5min otherwise. Infers OTC pick overall number for current-pick highlighting.
