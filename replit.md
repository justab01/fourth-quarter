...
- **Mobile App Features:**
    ...
    - **Sport Boards Overhaul:** All sport pages display sport-adaptive content with archetype-based section ordering and inline standings:
      ...
      - **Tennis:** This Week (active tournament hero card with GRAND SLAM badge for majors) → World Rankings (ATP/WTA with 150+ players, points, trend arrows) → Tournament Draw (collapsible bracket with seeds, match scores, live indicators) → Matches → Up Next → Top Athletes → Tournaments & Matches → News
      - **Golf:** Live Leaderboard (PGA/LPGA/LIV, top-10 with country flags, scores, MAJOR badges for majors, round detail) → Upcoming Tournaments (next 3-5 with MAJOR tags) → World Rankings → Top Athletes → Tournament Schedule → News. Supports PGA Tour, LPGA Tour, and LIV Golf with league-specific leaderboards.
      - **Combat/MMA:** Next Event card (UFC/Bellator/PFL with venue, date, main event matchup) → Division Rankings (UFC weight class selector: P4P, HW, LHW, MW, WW, LW, FW, BW, FLW + Women's divisions; filters 24 ESPN ranking groups) → Fights → Top Athletes → Fight Schedule → News. Leagues: UFC, Bellator, PFL, Boxing (pills filter). Bellator/PFL via ESPN `mma/bellator` and `mma/pfl` endpoints.
...
- **API Server Routes:**
    ...
    - `GET /api/sports/rankings/:league`: ESPN rankings for ATP, WTA, UFC, PGA, LPGA, F1, NASCAR, IRL (IndyCar). Returns `{ groups: [{ title, entries: [{ rank, name, points, record, trend, headshot, country }] }] }`. Cached 10min. F1 returns both Driver Standings and Constructor Standings groups with championship points.
    - `GET /api/sports/racing/schedule/:league`: Racing schedule/calendar for F1, NASCAR, IRL. Returns `{ league, nextRace: { name, date, circuit, location, countdownMs } | null, races: [{ id, name, date, circuit, location, status, results }] }`. Cached 5min.
