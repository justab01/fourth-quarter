...
- **API Server Routes:**
    ...
    - `GET /api/sports/rankings/:league`: ESPN rankings for ATP, WTA, UFC, PGA, F1, NASCAR, IRL (IndyCar). Returns `{ groups: [{ title, entries: [{ rank, name, points, record, trend, headshot, country }] }] }`. Cached 10min. F1 returns both Driver Standings and Constructor Standings groups with championship points.
