...
const card = StyleSheet.create({
  ...
  val: { fontSize: 22, fontWeight: "900", fontFamily: FONTS.bodyBold },
...
const ms = StyleSheet.create({
  ...
  stat: { color: C.text, fontSize: 15, fontWeight: "700", fontFamily: FONTS.bodySemiBold },
  barBg: { height: 6, backgroundColor: C.cardBorder, borderRadius: 3, overflow: "hidden" },
  barFill: { height: "100%", borderRadius: 3 },
  row: { flexDirection: "row", justifyContent: "space-between" },
  current: { color: C.textSecondary, fontSize: 12, fontFamily: FONTS.bodyMedium },
...
const glS = StyleSheet.create({
  ...
  introTitle: { color: C.text, fontSize: 30, fontWeight: "700", fontFamily: FONTS.bodyBold },
  introSubtitle: { color: C.textTertiary, fontSize: 14, fontFamily: FONTS.bodyMedium, marginTop: 2 },
  ...
  sectionTitle: {
    color: C.text, fontSize: 15, fontWeight: "700", fontFamily: FONTS.bodyBold,
  ...
  oppName: { color: C.text, fontSize: 14, fontWeight: "600", fontFamily: FONTS.bodySemiBold, marginLeft: 2 },
  ...
  resultBadgeText: { fontSize: 12, fontWeight: "700", fontFamily: FONTS.bodySemiBold },
  otBadge: {
...
function rowToEntry(row: GameLogRow): GameLogEntry {
  return {
...
function GameLogTab({
  ...
  const seasons = (league === "NBA" || league === "MLS")
    ? [2026, 2025, 2024]
    : [2025, 2024, 2023];
  ...
  const load = useCallback(async (s: number) => {
    ...
    setError(null);
    try {
      const url = getEspnGamelogUrl(playerName, league, String(s));
      if (!url) {
  ...
  const entries: GameLogEntry[] = React.useMemo(() => rows.map(rowToEntry), [rows]);
  const filteredEntries = React.useMemo(() => applyFilterToGames(entries, activeFilter), [entries, activeFilter]);
  const monthGroups = React.useMemo(() => groupByMonth(filteredEntries), [filteredEntries]);

  const seasonLabel = (league === "NFL" || league === "MLB")
    ? String(season)
  ...
  if (error) {
    return (
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 32, gap: 12 }}>
        ...
        <Text style={{ color: C.textSecondary, fontSize: 15, fontFamily: FONTS.bodySemiBold }}>
  ...
  if (rows.length === 0) {
    return (
      <View style={{ flex: 1, alignItems: "center", justifyContent: "center", padding: 32, gap: 12 }}>
        ...
        <Text style={{ color: C.textSecondary, fontSize: 15, fontFamily: FONTS.bodySemiBold }}>No games yet</Text>
  ...
  return (
    <ScrollView contentContainerStyle={{ paddingBottom: 60 }} showsVerticalScrollIndicator={false}>
      <View style={glS.introBlock}>
        <Text style={glS.introTitle}>Game Log</Text>
        <Text style={glS.introSubtitle}>{seasonLabel} Season</Text>
      </View>
      ...
      <View style={glS.seasonDropdownWrap}>
        ...
        >
          <Text style={glS.seasonDropdownText}>{seasonLabel}</Text>
          <Ionicons name={seasonDropdownOpen ? "chevron-up" : "chevron-down"} size={18} color={C.textSecondary} />
        </Pressable>
        {seasonDropdownOpen && (
          <View style={glS.seasonDropdownList}>
            {seasons.map(s => {
              ...
              const lbl = (league === "NFL" || league === "MLB") ? String(s) : `${s - 1}-${String(s).slice(2)}`;
              return (
                <Pressable
                  key={s}
                  onPress={() => {
      ...
      <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={glS.filterRow}>
        {GL_FILTERS.map(f => {
          const active = f === activeFilter;
          return (
            <Pressable
      ...
      {filteredEntries.length === 0 ? (
        ...
        <View style={{ paddingHorizontal: 16, gap: 0 }}>
          <Text style={glS.gamesCountText}>
            {filteredEntries.length} game{filteredEntries.length !== 1 ? "s" : ""}
          </Text>
          {monthGroups.map(group => (
            <View key={group.monthKey} style={glS.monthSection}>
              <View style={glS.monthHeaderRow}>
                <Text style={glS.monthHeader}>{group.month}</Text>
                <View style={glS.monthDivider} />
              </View>
              <MonthAvgCard
                month={group.month}
...
const indS = StyleSheet.create({
  ...
  gsCount: { fontSize: 26, fontWeight: "900", fontFamily: FONTS.bodyBold },
  ...
  methodCount: { fontSize: 26, fontWeight: "900", fontFamily: FONTS.bodyBold },
...
const compactS = StyleSheet.create({
  ...
  textWrap: { flex: 1 },
  name: { color: "#fff", fontSize: 16, fontWeight: "800", fontFamily: FONTS.bodyBold },
  sub: { color: "rgba(255,255,255,0.6)", fontSize: 12, fontFamily: FONTS.bodyMedium },
});
...
const styles = StyleSheet.create({
  ...
  heroAvatarInitial: { color: "#fff", fontSize: 52, fontWeight: "900", fontFamily: FONTS.bodyBold },
  topBar: {
    position: "absolute",
    left: 0, right: 0,
    flexDirection: "row", justifyContent: "space-between", alignItems: "center",
  ...
  backBtn: { width: 38, height: 38, alignItems: "center", justifyContent: "center", borderRadius: 10, backgroundColor: "rgba(23,32,42,0.46)" },
  followBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingHorizontal: 16, paddingVertical: 9, borderRadius: 20, backgroundColor: "rgba(23,32,42,0.52)", borderWidth: 1.5, borderColor: "rgba(255,248,232,0.28)" },
  followText: { color: "#fff", fontSize: 13, fontWeight: "700", fontFamily: FONTS.bodySemiBold },
  heroInfo: {
    position: "absolute",
    bottom: 16,
    left: 16,
  ...
  heroName: { color: "#fff", fontSize: 26, fontWeight: "900", fontFamily: FONTS.bodyBold, lineHeight: 30 },
  heroTeamLink: { color: "rgba(255,255,255,0.72)", fontSize: 13, fontFamily: FONTS.bodyMedium, marginTop: 3 },
  infoPill: { backgroundColor: "rgba(23,32,42,0.44)", borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3 },
  infoPillText: { color: "rgba(255,255,255,0.82)", fontSize: 11, fontFamily: FONTS.bodyMedium },
...
const table = StyleSheet.create({
  ...
  cell: { color: C.textSecondary, fontSize: 13, fontFamily: FONTS.body },
});

const bioTab = StyleSheet.create({
  name: { color: C.text, fontSize: 22, fontWeight: "900", fontFamily: FONTS.bodyBold, marginBottom: 4 },
